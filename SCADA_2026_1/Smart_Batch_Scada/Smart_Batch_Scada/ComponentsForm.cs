    ...
    public partial class ComponentsForm : Form
    {
        ...
        private void LoadComponents(string type)
        {
            using (var conn = new MySqlConnection(connString))
            {
                ...
                {
                    conn.Open();
                    string query = type == "AggMix" ?
                        @"SELECT m.mixture_id AS MixtureId, m.mixture_name AS Name, m.description AS Description, 
                          SUM(mc.percentage) AS TotalPercentage
                          FROM aggregate_mixtures m
                          JOIN mixture_components mc ON m.mixture_id=mc.mixture_id
                          GROUP BY m.mixture_id, m.mixture_name, m.description" :
                        "SELECT Code, Description, UOM, Gravity, Color FROM Components WHERE Type=@Type";
        ...
        private void btnEdit_Click(object sender, EventArgs e)
        {
            ...
            if (currentFilterType == "AggMix")
            {
                int mixtureId = Convert.ToInt32(dataGridView1.SelectedRows[0].Cells["MixtureId"].Value);
                using var form = new AggregateMixtureForm(mixtureId);
                if (form.ShowDialog() == DialogResult.OK)
                    LoadComponents(currentFilterType);
            }
            else
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            ...
            {
                int mixtureId = Convert.ToInt32(dataGridView1.SelectedRows[0].Cells["MixtureId"].Value);
                if (MessageBox.Show("Delete this mixture?", "Confirm", MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    using var conn = new MySqlConnection(connString);
                    conn.Open();
                    using var del1 = new MySqlCommand("DELETE FROM mixture_components WHERE mixture_id=@mid", conn);
                    del1.Parameters.AddWithValue("@mid", mixtureId);
                    del1.ExecuteNonQuery();

                    using var del2 = new MySqlCommand("DELETE FROM aggregate_mixtures WHERE mixture_id=@mid", conn);
                    del2.Parameters.AddWithValue("@mid", mixtureId);
        ...
        private void btnImport_Click(object sender, EventArgs e)
        {
            ...
            foreach (var line in lines)
            {
                ...
                if (currentFilterType == "AggMix") continue; // skip AggMix import
        ...
        private void btnExport_Click(object sender, EventArgs e)
        {
            ...
            foreach (DataGridViewRow row in dataGridView1.Rows)
            {
                ...
                if (currentFilterType == "AggMix")
                    sw.WriteLine($"{row.Cells["MixtureId"].Value},{row.Cells["Name"].Value},{row.Cells["Description"].Value},{row.Cells["TotalPercentage"].Value}");
                else
        ...
        private void btnPrint_Click(object sender, EventArgs e)
        {
            ...
            doc.PrintPage += (s, ev) =>
            {
                ...
                {
                    if (row.IsNewRow) continue;
                    string line = currentFilterType == "AggMix" ?
                        $"{row.Cells["Name"].Value} ({row.Cells["TotalPercentage"].Value}%)" :
                        $"{row.Cells["Code"].Value} - {row.Cells["Description"].Value}";
        ...
        private void btnAggMix_Click(object sender, EventArgs e) { currentFilterType = "AggMix"; LoadComponents(currentFilterType); HighlightActiveButton(btnAggMix); }
    }
}
