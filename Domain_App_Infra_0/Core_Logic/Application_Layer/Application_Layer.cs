  ...
  namespace SCADA.Application
{
  ...
  public class DestinationService
    ...
    /// <start>
    /// edit this function "MaterialDataService" to match the new structure
    public class MaterialDataService
    {
        private readonly GeneralFunctions<MaterialsData> repository;

        public MaterialDataService(GeneralFunctions<MaterialsData> repo)
        {
            repository = repo;
        }
        ...
        public OperationResults AddNewData(MaterialsData m)
        {
            ...
            {
                repository.ADD(m);
                return new OperationResults { Success = true, Message = "Material saved successfully." };
            }
            catch (Exception ex)
            {
                return new OperationResults { Success = false, Message = $"Failed to save material: {ex.Message}" };
            }
        }
        ...
        public OperationResults RemoveMaterial(MaterialsData m)
        {
            ...
            {
                repository.Delete(m);
                return new OperationResults { Success = true, Message = "Material deleted successfully." };
            }
            catch (Exception ex)
            {
                return new OperationResults { Success = false, Message = $"Failed to delete material: {ex.Message}" };
            }
        }

        // ✅ Delete by code (called from ComponentsForm)
        public OperationResults DeleteMaterialByCode(string code)
        {
            var foundMaterial = repository.Find(code);
            if (foundMaterial == null)
                return new OperationResults { Success = false, Message = $"Material with code '{code}' not found." };

            return RemoveMaterial(foundMaterial);
        }
        ...
        public OperationResults EditMaterial(MaterialsData m)
        {
            ...
            {
                repository.Update(m);
                return new OperationResults { Success = true, Message = "Material updated successfully." };
            }
            catch (Exception ex)
            {
                return new OperationResults { Success = false, Message = $"Failed to update material: {ex.Message}" };
            }
        }
        ...
        public OperationResults<List<MaterialsData>> GetAllMaterials()
        {
            ...
            {
                return OperationResults<List<MaterialsData>>.failure($"Failed to retrieve materials: {ex.Message}");
            }
        }
        ...
        public OperationResults<MaterialsData> FindMaterial(int id)
        {
            ...
            if (result == null)
                return OperationResults<MaterialsData>.failure($"Material with ID {id} not found.");

            return OperationResults<MaterialsData>.success(result, $"Material with ID {id} found.");
        }

        // -------------------------------------------------------
        // Find material by Code
        // -------------------------------------------------------
        public OperationResults<MaterialsData> FindMaterial(string code)
        {
            var result = repository.Find(code);
            if (result == null)
                return OperationResults<MaterialsData>.failure($"Material with code '{code}' not found.");

            return OperationResults<MaterialsData>.success(result, $"Material with code '{code}' found.");
        }
    }
    ...
    public class Calculations
    {
        ...
        public Calculations(FormulaService formulaService)
        {
            this.formulaService = formulaService;
        }
        //--------------------------
        // Materials Calculations:
        public float CalculateConsumption_Quantity(float initial_qty, float final_qty)
        {
            return initial_qty - final_qty;
        }
        public float CalculateTotal_Cost(float cost_per_unit, float qty)
        {
            return cost_per_unit * qty;
        }
        public (float Efficiency, float Deviation) CalculatEfficiency(float target, float actual)
        {
            float deviation = actual - target; 
            // if positive actual > target, if negative actual < target
            return (Math.Abs((1-(deviation/target))*100.0f), deviation);
        }
        public float CalculateCurrent_Qty(float initial_Qty, float consumed_Qty, float Received_Qty)
        {
            float Current_Qty = initial_Qty + Received_Qty - consumed_Qty;
            return Current_Qty;
        }
        //--------------------------------
        // Calculate Duration:
        public (double days, double hours, double months) CalculateDuration_Days(DateTime start_time, DateTime end_time)
        {
            double months = 0.0;
                TimeSpan duration = end_time - start_time;
                double days = duration.TotalDays;
                double hours = duration.Hours;
            // Modify it to adjust the number of days based on different months
            if (days >= 30)
            {
                months = days / 30.0;
            }
        ...
        //---------------------------------------
        public float CalculateTotalSalary(float basic_salary, float bonus)
        {
            float totalsalary = basic_salary * bonus + basic_salary;
            return totalsalary;
        }
        //----------------------------
        //
        public float MoistureCorrection(float Water_Weight, float Sand_Weight, float Sand_Moisture)
        {
            float Corrected_Water = Water_Weight - (Sand_Moisture * Sand_Weight);
            return Corrected_Water;
        }
        //--------------------------------
        // Calculate total production weight per day:
        public float Total_Production_Weight(OrderService order, DateOnly date)
        {
            var days = date.Day;
            this.orderService = order;
            // get orders finished in that day and check if thier state is finished or not.
            var order_results = orderService.GetAllOrders();
            
            // then sum their weights.
        ...
        //----------------------------------
        // Calculate Alarm Frequesncy:

        //-------------------------------------
        // 
        //--------------------------------------
        //Calculate Average Batch Cycle Time:

        //------------------------------------
        // PLC Batch Calculations:
            // Calc the target weight:
        private float CalculateTarget_Weight(string formula_code, float concrete_weight, string material_name)
        {
            var result = formulaService.FindFormula(formula_code);
            if (result.data == null)
            {
                throw new Exception("no formula exists with the given formula code");
            }
            var sand_portion = result.data.getSandtPortion();
            var gravel_portion = result.data.getGravelPortion();
            var cement_portion = result.data.getCementPortion();
            var water_cement_ratio = result.data.getWaterCementRatio();
            var adx1_portion = result.data.getADX1Portion();
            var adx2_portion = result.data.getADX2Portion();
            var Ice_portion = result.data.getIcePortion();
            var Color_portion = result.data.getColorPortion();
            float total_portion = sand_portion + gravel_portion + cement_portion +adx1_portion + adx2_portion + Ice_portion + Color_portion;
            switch (material_name)
            {
                case "Sand": return (sand_portion/total_portion)*concrete_weight;
                case "Gravel": return (gravel_portion/total_portion)*concrete_weight;
                case "Cement": return (cement_portion/total_portion)*concrete_weight;
                case "Water": return water_cement_ratio* (cement_portion / total_portion) * concrete_weight; // must be cement quantity
                case "ADX1": return (adx1_portion / total_portion) * concrete_weight;
                case "ADX2": return (adx2_portion / total_portion) * concrete_weight;
                case "Color": return (Color_portion / total_portion) * concrete_weight;
                case "Ice": return (Ice_portion / total_portion) * concrete_weight;
                default: return 0;
            }
        }
            //---------------------
            // Fraction Calculation:
        public (int no_of_cycles, float middle_batch_concrete_weight, float last_batch_concrete_weight, bool fraction_calc_flag) Fraction_Calculation(float mixer_capacity, float total_requested_volume, float concrete_density)
        {
            int no_of_cycles = 0;
            float middle_concrete_weight = 0;
            float last_concrete_weight = 0;
            bool calc_flag = false;
            if(total_requested_volume <= mixer_capacity)
            {
                no_of_cycles = 1;
                middle_concrete_weight  = concrete_density*total_requested_volume;
                last_concrete_weight = middle_concrete_weight;
              
            }
            else
            {
                no_of_cycles = (int)Math.Ceiling(total_requested_volume / mixer_capacity);
                var no_of_full_batches = (int) Math.Truncate(total_requested_volume / mixer_capacity);
                var last_batch_volume = total_requested_volume - (no_of_full_batches * mixer_capacity);
                if(last_batch_volume >0)
                {
                    var middle_batch_volume = (total_requested_volume - last_batch_volume) / (no_of_cycles-1);
                    middle_concrete_weight = middle_batch_volume * concrete_density;
                    last_concrete_weight = last_batch_volume * concrete_density;
                }
                else
                {
                    var middle_batch_volume = total_requested_volume / mixer_capacity;
                    last_batch_volume = middle_batch_volume;
                    middle_concrete_weight = middle_batch_volume * concrete_density;
                    last_concrete_weight = last_batch_volume * concrete_density;
                }
            }
            calc_flag = true;
            return (no_of_cycles, middle_concrete_weight, last_concrete_weight, calc_flag);
        }
           //-----------------------------
          // Target Weight Calc.per batch:
        public (float Sand_Weight,float Gravel_Weight, float Cement_Weight, float Water_Weight, float ADX1_Weight, float ADX2_Weight, float Color_Weight, float Ice_Weight, bool Target_Weight_Flag, string error) Calc_target_Weight_per_batch(int no_of_cycles, int batch_counter, float middle_batch_concrete_weight, float last_batch_concrete_weight, string formula_code)
        {
            List<string> material_names = new List<string> { "Sand, Gravel, Cement, Water, ADX1, ADX2, Color, Ice"};
            float sand_weight = 0.0f;
            float gravel_weight = 0.0f;
            float cement_weight = 0.0f;
            float water_weight = 0.0f;
            float adx1_weight = 0.0f;
            float adx2_weight = 0.0f;
            float color_weight = 0.0f;
            float ice_weight = 0.0f;
            bool target_weight_flag = false;
            string Error = string.Empty; // by defualt
            try
            {
                if (batch_counter < no_of_cycles)
                {
                    foreach (string name in material_names)
                    {
                        switch (name)
                        {
                            case "Sand":
                                sand_weight = CalculateTarget_Weight(formula_code, middle_batch_concrete_weight, name);
                                break;
                            case "Gravel":
                                gravel_weight = CalculateTarget_Weight(formula_code, middle_batch_concrete_weight, name);
                                break;
                            case "Cement":
                                cement_weight = CalculateTarget_Weight(formula_code, middle_batch_concrete_weight, name);
                                break;
                            case "Water":
                                water_weight = CalculateTarget_Weight(formula_code, middle_batch_concrete_weight, name);
                                break;
                            case "ADX1":
                                adx1_weight = CalculateTarget_Weight(formula_code, middle_batch_concrete_weight, name);
                                break;
                            case "ADX2":
                                adx2_weight = CalculateTarget_Weight(formula_code, middle_batch_concrete_weight, name);
                                break;
                            case "Color":
                                color_weight = CalculateTarget_Weight(formula_code, middle_batch_concrete_weight, name);
                                break;
                            case "Ice":
                                ice_weight = CalculateTarget_Weight(formula_code, middle_batch_concrete_weight, name);
                                break;
                        }
                    }
                    target_weight_flag = true;
                }
                else if (batch_counter == no_of_cycles)
                {
                    foreach (string name in material_names)
                    {
                        switch (name)
                        {
                            case "Sand":
                                sand_weight = CalculateTarget_Weight(formula_code, last_batch_concrete_weight, name);
                                break;
                            case "Gravel":
                                gravel_weight = CalculateTarget_Weight(formula_code, last_batch_concrete_weight, name);
                                break;
                            case "Cement":
                                cement_weight = CalculateTarget_Weight(formula_code, last_batch_concrete_weight, name);
                                break;
                            case "Water":
                                water_weight = CalculateTarget_Weight(formula_code, last_batch_concrete_weight, name);
                                break;
                            case "ADX1":
                                adx1_weight = CalculateTarget_Weight(formula_code, last_batch_concrete_weight, name);
                                break;
                            case "ADX2":
                                adx2_weight = CalculateTarget_Weight(formula_code, last_batch_concrete_weight, name);
                                break;
                            case "Color":
                                color_weight = CalculateTarget_Weight(formula_code, last_batch_concrete_weight, name);
                                break;
                            case "Ice":
                                ice_weight = CalculateTarget_Weight(formula_code, last_batch_concrete_weight, name);
                                break;
                        }
                    }
                    target_weight_flag = true;
            ...
            catch (Exception e) 
            {
                ...
                return (sand_weight, gravel_weight, cement_weight, water_weight, adx1_weight, adx2_weight, color_weight, ice_weight, target_weight_flag, Error);
        }
            //--------------------------------
        //---------------------------------------------------------
    }
