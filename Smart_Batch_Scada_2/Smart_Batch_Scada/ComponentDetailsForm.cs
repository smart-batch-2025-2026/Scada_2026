...
namespace Smart_Batch_Scada
{
    public partial class ComponentDetailsForm : Form
    {
        ...
        public ComponentDetailsForm(string? codeToEdit = null, string? filterType = null)
        {
            InitializeComponent();
            _editCode = codeToEdit;
            _currentFilterType = filterType;

            // Removed "Aggregates Mix" from the list
            cmbComponentType.Items.AddRange(new string[]
            {
                "Aggregate","Cement","Water","Additive","Color","Adding"
            });

            // Set initial selection based on filter type
            if (!string.IsNullOrEmpty(_currentFilterType))
            {
                cmbComponentType.SelectedItem = GetComboComponentType(_currentFilterType);
            }
            else
            {
            ...
            // If editing existing component, load its data
            if (!string.IsNullOrEmpty(_editCode))
            {
                LoadComponent(_editCode);
            }
        }
